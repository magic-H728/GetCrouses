...
def addCrouse(jx0404id, kcid, session, type):
    # print(id)
    # headers = {
    #     # 定期更换Cookie
    ...
    # }
    print(jx0404id)
    url = 'https://jwxt.sztu.edu.cn/jsxsd/xsxkkc/bxqjhxkOper?kcid='
    url += str(kcid) + '&cfbs=null' + '&jx0404id=' + \
...
def getID(res):
    ...
    list = res.find_all('a', href="#")[0]
    return list.attrs['onclick'].split("'")[1]


def getCrouse(session, CourseName, TeacherName, Day, Time, type):
    # 编造cookies 伪装成浏览器
    # headers = {
    #     # 定期更换Cookie
    #     'Cookie': "JSESSIONID=F7F851EE3C65E1A1750F8768E4C03E63; SELF_SERVICE_SID=96294F382E2A4D1EBC260D5042CF4EB5; SERVERID=125; JSESSIONID=B3A128BA5D1FDA6594F79683F48A35E5"
    ...
    # 拼接url
    urls = 'https://jwxt.sztu.edu.cn/jsxsd/xsxkkc/xsxkBxqjhxk?kcxx=' + kcxx + '&skls=' + skls + '&skxq=' + skxq + '&skjc=' + skjc + '&sfym=' + str(
    ...
    for crouse in res['aaData']:
        print("课程名称：", crouse['kcmc'] + " " + str(crouse['fzmc']), "剩余主选人数", crouse['syzxwrs'],
              "剩余非主选人数",
              crouse['syfzxwrs'], "授课老师:", crouse['skls'], 'count:', str(crouse['count']))
        print('----------------------------------')
